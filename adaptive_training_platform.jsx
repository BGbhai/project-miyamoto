...
function createGeminiParseError(requestType = 'response', rawText = '') {
  const err = new Error(`Gemini ${requestType} parse error: invalid JSON payload.`);
...
function parseGeminiJson(text, requestType = 'response') {
  ...
  if (!clean) throw createGeminiParseError(requestType, text);
  ...
  try {
    ...
    if (extracted) {
      try {
        ...
        throw createGeminiParseError(requestType, clean);
      }
    }
    throw createGeminiParseError(requestType, clean);
  }
...
function tryParseCoachPayload(text, workout = []) {
  const parsed = parseGeminiJson(text, 'response');
  return normalizeCoachPayload(parsed, workout);
}
...
async function callGeminiCoachWithRepair({ apiKey, endpoint, primaryBody, workout }) {
  const primaryData = await callGeminiWithRetry({
    apiKey,
    endpoint,
    body: primaryBody,
...
function buildFallbackCoachingNotes(workout, userContext = {}) {
  ...
  return {
    sessionFocus: `Prioritize crisp technique and consistent effort for this ${typeLabel} session.`,
    watchOut: 'Scale load or complexity if form degrades or pain appears.',
    exercises,
  };
}
...
${countRule}
{
  ...
  return callGeminiCoachWithRepair({ apiKey, endpoint, primaryBody: body, workout });
}
...
function migrateSessionLog(log, idx) {
  ...
  const normalizedPrescription = { sessionVariant: null, selectionMeta: null, ...legacyPrescription };
...
function appReducer(state, action) {
  switch (action.type) {
    ...
    case 'UPDATE_AI_CONFIG': {
      const mergedAI = { ...state.aiConfig, ...(action.payload || {}) };
      const normalizedModel = normalizeGeminiModel(mergedAI.model);
      return {
        ...state,
        aiConfig: {
    ...
    case 'SET_AI_COACHING': {
      const { notes, loading, error, warning } = action.payload || {};
      return {
        ...state,
        aiConfig: {
          ...state.aiConfig,
    ...
    case 'START_SESSION_FROM_PLAN': {
      ...
      return {
        ...
        aiConfig: { ...state.aiConfig, coachingNotes: null, coachingLoading: false, coachingError: null, coachingWarning: null },
      };
    }
...
function TrainView({ state, dispatch }) {
  ...
  useEffect(() => {
    ...
    dispatch({ type: 'SET_AI_COACHING', payload: { loading: true, error: null, warning: null } });
    const userContext = {
      name: state.profile.name || 'Athlete',
      age: state.profile.age || '?',
      bodyweight: state.profile.bodyweight || '?',
    ...
    callGeminiAPI(
      ...
      dispatch({ type: 'SET_AI_COACHING', payload: { error: err.message, loading: false, warning: null } });
    });
